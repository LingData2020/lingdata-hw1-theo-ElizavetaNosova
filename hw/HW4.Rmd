...
Выбираю тест хи-квадрат, так как он подходит для анализа корреляции переменных, одна из которых категориальная, а другая числовая


```{r}
res <- chisq.test(stimuli_data$TypeOfStimuli, stimuli_data$`mean(RTlog)`)
res
...
К результатам хи-квадрат теста применить метод не получилось, поэтому воспользуюсь aov и применю к его результатам.
